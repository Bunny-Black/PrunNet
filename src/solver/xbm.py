...
class XBM:
    def __init__(self, memory_size, embedding_dim, device):
        self.K = memory_size
        self.feats = torch.zeros(self.K, embedding_dim).to(device).to(torch.float)
        self.targets = -torch.ones(self.K, dtype=torch.long).to(device)
        self.img_ids = -torch.ones(self.K, dtype=torch.long).to(device)
        self.ptr = 0
    ...
    def get(self):
        if self.is_full:
            return self.feats, self.targets, self.img_ids
        else:
            return self.feats[:self.ptr], self.targets[:self.ptr], self.img_ids[:self.ptr]

    def enqueue_dequeue(self, feats, targets, img_ids=None):
        q_size = len(targets)
        if self.ptr + q_size > self.K:
            self.feats[-q_size:] = feats
            self.targets[-q_size:] = targets
            if img_ids is not None:
                self.img_ids[-q_size:] = img_ids
            self.ptr = 0
        else:
            self.feats[self.ptr: self.ptr + q_size] = feats
            self.targets[self.ptr: self.ptr + q_size] = targets
            if img_ids is not None:
                self.img_ids[self.ptr: self.ptr + q_size] = img_ids
            self.ptr += q_size
