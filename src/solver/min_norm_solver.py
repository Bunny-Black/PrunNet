...
class MinNormSolver:
        ...
        for i in range(len(vecs)):
            for j in range(i+1,len(vecs)):
                if (i,j) not in dps:
                    ...
                    dps[(i, j)]=(vecs[i] * vecs[j]).sum(dim=(1, 2, 3)).sum().data
                    dps[(j, i)] = dps[(i, j)]
                if (i,i) not in dps:
                    # dps[(i, i)] = 0.0
                    # for k in range(len(vecs[i])):
                    #     dps[(i,i)] += torch.mul(vecs[i][k], vecs[i][k]).sum()
                    dps[(i,i)]=(vecs[i] * vecs[i]).sum(dim=(1, 2, 3)).sum().data    # 二者在小数点后面的精度不一致
                if (j,j) not in dps:
                    # dps[(j, j)] = 0.0   
                    # for k in range(len(vecs[i])):
                    #     dps[(j, j)] += torch.mul(vecs[j][k], vecs[j][k]).sum().data
                    dps[(j, j)]=(vecs[j] * vecs[j]).sum(dim=(1, 2, 3)).sum().data
                    # print("dps=",dps[(j, j)])
                    # print("tmp=",tmp)
    ...
    def _projection2simplex(y,device):
        """
        Given y, it solves argmin_z |y-z|_2 st \sum z = 1 , 1 >= z_i >= 0 for all i
        """
        m = len(y)
        # sorted_y = torch.flip(torch.sort(y), dims=[0])
        sorted_values, sorted_indices = torch.sort(y)

        # 对排序后的张量进行翻转
        sorted_y = torch.flip(sorted_values, dims=[0])
        ...
        for i in range(m-1):
            ...
            if tmax > sorted_y[i+1]:
                tmax_f = tmax
                break
        return torch.maximum(y - tmax_f, torch.zeros(y.shape).to(device))
    
    def _next_point(cur_val, grad, n,device):
        ...
        next_point = MinNormSolver._projection2simplex(next_point,device)
        return next_point

    def find_min_norm_element(vecs,device):
        """
        ...
        n=len(vecs)
        sol_vec = torch.zeros(n).to(device)
        ...
        grad_mat = torch.zeros((n,n)).to(device)
        for i in range(n):
            for j in range(n):
        ...
        while iter_count < MinNormSolver.MAX_ITER:
            ...
            new_point = MinNormSolver._next_point(sol_vec, grad_dir, n,device)
