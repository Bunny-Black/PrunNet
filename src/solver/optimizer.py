...
def make_optimizer(
    ...
    if train_params.WEIGHT_DECAY > 0:
        if train_params.OPTIMIZER == 'adamw':# 就是config的设置参数
            if train_params.BACKBONE_MULTIPLIER == 1.:
                ...
                no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight']
                no_backbone = ['adapter', "projection", "projection_cls"]
                optimizer_grouped_parameters = [
                    {'params': [p for n, p in params
                                if not any(nd in n for nd in no_decay) and not any(nb in n for nb in no_backbone)],
                    'weight_decay': 0.01,
                    'lr': lr * train_params.BACKBONE_MULTIPLIER},
                    {'params': [p for n, p in params
                                if any(nd in n for nd in no_decay) and not any(nb in n for nb in no_backbone)], 
                    'weight_decay': 0.0,
                    'lr': lr * train_params.BACKBONE_MULTIPLIER},
                    {'params': [p for n, p in params
                                if not any(nd in n for nd in no_decay) and any(nb in n for nb in no_backbone)],
                    'weight_decay': 0.01,
                    'lr': lr},
                    {'params': [p for n, p in params
                                if any(nd in n for nd in no_decay) and any(nb in n for nb in no_backbone)], 
                    'weight_decay': 0.0,
                    'lr': lr},
                ]
            optimizer = AdamW(
