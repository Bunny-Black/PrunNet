...
class ImageNet_list_Dataset(BaseDataset):
    def __init__(self, *args, **kwargs):
        super(ImageNet_list_Dataset, self).__init__(*args, **kwargs)
        assert self.split in {"train"}

    def set_paths_and_labels(self):
        self.paths = [self.data_dir+"/train/"+line.split(' ')[0] for line 
                        in open(os.path.join(self.data_dir, self.cfg.DATA.TRAIN_IMG_LIST))]
