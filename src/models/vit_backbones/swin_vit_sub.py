...
class WindowAttention_sub(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.):
        ...
        self.proj = SubnetLinear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)

        trunc_normal_(self.relative_position_bias_table, std=.02)
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, x, mask: Optional[torch.Tensor] = None, sparsity=0):
        """
        Args:
        ...
        """
        B_, N, C = x.shape
        qkv = self.qkv(x,sparsity).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class SwinTransformerBlock_sub(nn.Module):
    ...
    def __init__(self, cfg, dim, input_resolution, num_heads, window_size=7, shift_size=0,
    ...
    def forward(self, x, sparsity):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=self.attn_mask,sparsity=sparsity)  # nW*B, window_size*window_size, C
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, cfg, dim, input_resolution, depth, num_heads, window_size,
        ...
        # build blocks
        blocks = [
        SwinTransformerBlock_sub(
            cfg=cfg, dim=dim, input_resolution=input_resolution, num_heads=num_heads, window_size=window_size,
...
class MySequential(nn.Module):
    ...
    def forward(self, x, sparsity):
        for module in self.block_list:
            if isinstance(module,(SubnetConv2d,SubnetLinear,BasicLayer,SwinTransformerBlock_sub)):
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, weight_init='', cfg=None,**kwargs):
        super().__init__()
        ...
        self.patch_embed = PatchEmbed_sub(
            self.cfg,img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
        ...
        self.layers = MySequential(*layers)
...
class Our_Swin_VIT(nn.Module):
    def __init__(self, cfg, pretrained=False):
        super(Our_Swin_VIT, self).__init__()
        self.cfg = cfg
        self.sparsity = 0
        ...
        self.enc = _create_swin_transformer(self.cfg.MODEL.DATA.FEATURE, pretrained, self.model_cfg, **self.model_kwargs)
