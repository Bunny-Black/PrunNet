...
CONFIGS = {
    ...
    'vit-s': configs.get_vit_small_config(),
...
class Encoder_ours(nn.Module):
    def __init__(self, config, img_size, vis, in_channels=3,):
        ...
        if config.patches.get("grid") is not None:
            ...
            patch_size = _pair(config.patches["size"])
            n_patches = (img_size[0] // patch_size[0]) * (img_size[1] // patch_size[1])
        self.pos_embed = nn.Parameter(torch.zeros(1, n_patches+1, config.hidden_size))
...
class VisionTransformer(nn.Module):
    def __init__(
        self, cfg, vit_cfg,
        img_size=224, num_classes=24393, vis=False
    ):
        super(VisionTransformer, self).__init__()
        self.cfg = cfg 
        self.vit_cfg = vit_cfg
        self.num_classes = num_classes
        self.feat_dim = vit_cfg.hidden_size
        self.classifier = self.vit_cfg.classifier
        # self.transformer = Transformer(self.vit_cfg, img_size, vis)
        self.enc = Embeddings(self.vit_cfg, img_size=img_size)
        self.encoder = Encoder(self.vit_cfg, vis)        
        if self.cfg.MODEL.PROJECTION_LAYERS!=-1:
            self.setup_projector(self.cfg.MODEL)
        else:
            self.projector = nn.Identity()
        self.setup_head(self.cfg.MODEL)
        # self.head = Linear(config.hidden_size, num_classes) if num_classes > 0 else nn.Identity()
        
    def setup_projector(self, model_cfg):
        self.projector = MLP(
            input_dim=self.vit_cfg.hidden_size,
    ...
    def setup_head(self, model_cfg):
        input_dim = model_cfg.PROJECTION_DIM if model_cfg.PROJECTION_LAYERS!=-1 else self.vit_cfg.hidden_size
    ...
    def forward(self, x, vis=False, return_feature=True):
...
def build_vit_ours(cfg):
    vit_config = CONFIGS[cfg.MODEL.DATA.FEATURE]
    model = VisionTransformer(cfg,vit_config,img_size=224,num_classes=cfg.MODEL.DATA.NUMBER_CLASSES)
    return model
