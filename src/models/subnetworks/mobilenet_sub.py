...
class ConvBNReLU(nn.Module):  # depthwise conv + BN + ReLU6
    def __init__(self, cfg, in_channel, out_channel, kernel_size=3, stride=1, groups=1):
        super(ConvBNReLU, self).__init__()
        self.cfg = cfg
        # 计算 padding 大小
        ...
        # 定义各层
        self.conv = SubnetConv2d(cfg=self.cfg, in_channels=in_channel, out_channels=out_channel, kernel_size=kernel_size, stride=stride, padding=padding, bias=False, groups=groups)
        self.bn = nn.BatchNorm2d(out_channel)
        self.relu6 = nn.ReLU6(inplace=True)

    def forward(self, x, sparsity):
        # 前向传播
        x = self.conv(x,sparsity)  # 进行卷积
        x = self.bn(x)    # 进行批归一化
...
class InvertedResidual(nn.Module):#逆向残差结构 
    def __init__(self, cfg, in_channel, out_channel, stride, expand_ratio): 
        ...
        layers.extend([ 
            # 3x3 depthwise conv 
            ConvBNReLU(cfg,hidden_channel, hidden_channel, stride=stride, groups=hidden_channel), 
            # 1x1 pointwise conv(linear)第二个普通1x1卷积，降维 
            SubnetConv2d(cfg,hidden_channel, out_channel, kernel_size=1, bias=False), 
            nn.BatchNorm2d(out_channel),#BN 
        ]) 

        self.conv = MySequential(*layers)#将上面的集成到一起 

    def forward(self, x, sparsity):#前向传播过程，直接用上面弄好的conv层 
        if self.use_shortcut: 
            return x + self.conv(x, sparsity) 
        else: 
            return self.conv(x, sparsity) 
...
class MySequential(nn.Module):
    ...
    def forward(self, x, sparsity):
        for module in self.block_list:
            if isinstance(module,(SubnetConv2d,SubnetLinear,ConvBNReLU,InvertedResidual)):
...
class MobileNetV2(nn.Module):#整体的网络 
    def __init__(self, cfg, num_classes=1000, alpha=1.0, round_nearest=8): 
        ...
        features.append(ConvBNReLU(self.cfg, 3, input_channel, stride=2)) 
        ...
        for t, c, n, s in inverted_residual_setting: 
            ...
            for i in range(n):#构建n层逆向残差 
                ...
                features.append(block(self.cfg, input_channel, output_channel, stride, expand_ratio=t)) 
                input_channel = output_channel 
        self.feat_dim = last_channel
        # building last several layers，构建逆向残差后面的1x1普通卷积 
        features.append(ConvBNReLU(self.cfg, input_channel, last_channel, 1)) 
    ...
    def forward(self,x,sparsity=0,mask=None,return_feature=True):#前向传播过程 
        x = self.features(x,sparsity)#获得特征图 
