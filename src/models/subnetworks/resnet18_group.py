...
def subnet_conv3x3(cfg,in_planes, out_planes, stride=1, sparsity=0.5, groups=1):
    return SubnetConv2d(cfg,in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=False, sparsity=sparsity, groups=groups)

def subnet_conv7x7(cfg,in_planes, out_planes, stride=1, sparsity=0.5, groups=1):
    return SubnetConv2d(cfg,in_planes, out_planes, kernel_size=7, stride=stride,
                     padding=(3,3), bias=False, sparsity=sparsity, groups=groups)
...
class SubnetBasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, sparsity=0.5, name="", cfg=None):
        ...
        self.affine = True
        self.conv1 = subnet_conv3x3(cfg,in_planes, planes, stride, sparsity=sparsity, groups=GROUP_NUM)
        if self.affine:
        ...
        self.conv2 = subnet_conv3x3(cfg,planes, planes, sparsity=sparsity, groups=GROUP_NUM)
        if self.affine:
        ...
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = 1
            self.conv3 = SubnetConv2d(cfg,in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False, sparsity=sparsity, groups=GROUP_NUM)
            if self.affine:
...
class SubnetResNet_group(nn.Module):
    def __init__(self, block, num_blocks, nf, sparsity, cfg, model_cfg):
        super(SubnetResNet_group, self).__init__()
        ...
        self.conv1 = subnet_conv7x7(cfg,3, nf * 1, (2,2), sparsity=sparsity, groups=GROUP_NUM)
        if True:
...
def SubnetResNet18_group(cfg,model_cfg, nf=224, sparsity=0.5):
    return SubnetResNet_group(SubnetBasicBlock, [2, 2, 2, 2], nf, sparsity,cfg,model_cfg)
